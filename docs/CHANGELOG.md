...
3. **导出功能修复**
   - 修复 PDF 导出中文乱码问题（改用浏览器打印 API）
   - 修复 PDF 文字显示范围问题
   - 修复 DOCX 导出背景颜色不显示问题
   - 修复 Tauri 环境下 PDF 导出的窗口弹出问题（改用当前窗口打印）
   - 优化 DOCX 导出的段落样式和间距

### 技术改进

4. **依赖包更新**
   - 新增 `html2canvas` - HTML 转图片库（用于未来的截图导出功能）
   - 新增 `docx` - DOCX 文档生成库
   - 移除 `jspdf` 和 `@types/jspdf`（改用浏览器原生打印 API）
