...
- 新增 Git 跟踪的文档库元数据 `.config/library.json`
- 新增 Favorites、Recent、Archived、Trash 逻辑分组
- 新增回收站恢复与永久删除流程
- 新增 Markdown 导入与 Markdown 导出
- 新增命令面板（`Ctrl+K` / `Cmd+K`）
- 新增工作区级提交历史时间线
- 设置页新增远程仓库可编辑输入、工作区自动提交间隔和仓库状态展示

### 文档对齐

- 将搜索、导出从“待实现”调整为“已实现”
- 统一同步策略说明：当前为冷启动/前台恢复/手动同步/关闭前同步与失败重试，不再宣称“每次提交后立即 push”
